...
// JS DOC
/**
 * 입력 받은 아이디와 비밀번호가 맞난지 확인후 적절한 알림
...
//loginUser();
// switch 문
//switch의 안과 case 의 조건으로 비교식 들어갈 수 없음
...
switch (x) {
  ...
  case 4:
    console.log("x는 4입니다");
  //break 작성하지 않으면 case 4를 실핼하고 싶을때 case 5까지 모두 실행
  //해당 스코프를 빠저나가지 못하는 것
  //원하는 코드가 실행된 이후 꼭 break 사용해
  case 5:
    console.log("x는 5입니다");
    break;
  default:
    console.log("x는 3, 4, 5,가 아는 다른 값입니다");
}
//if 문으로 작성한 성적 게산하는 성적 프로그렘 switch 문 사용하도록 수정
//switch(number){
//case 100:
//console.log('A')
// break
...
// break
//}
// 위처럼 작성하는 경우 100 90 ....이런 값에 아니면 원하는 동작을 하지 않음
// 이럴 땐 점수를 10어로 나눈 몫을 찾ㅊ아와 number으로 전달, case 값도 변경
/* 
1 변수 x의 값과 첫 번째 case 문의 값 3을 비교
2. 두 값의 동일히다: 콜론 코드가 실행
3.break 문을 통해 해당 스코프에서 빠져나감
4. 모든 case 믄의 값과 x 의 값 다르다면 defualt 믄 다음의 코드가 실핼 
...
num % 2 === 1 ? "홀수" : "짝수";
let fruit = "banana";
console.log('fruit === "banana" ? "바나나" : "바나나가 아님니다.');
